...
// ---------------------------------------------------------------------------
// Placements and candidates ΓÇö human-readable times for the receptionist
// ---------------------------------------------------------------------------
...
export const scheduleSegmentSchema = z.object({
  ...
  /** Human label for this step (Consult, Inject, Scan, …). Null for delays. */
...
export const scheduleAppointmentSchema = z.object({
  id: z.uuid(),
  patientId: z.uuid(),
  /** Shown on the grid; authenticated staff only — never in logs. */
...
export const resourceAvailabilitySchema = z.object({
  resourceId: z.uuid(),
  /** Slot indices (0-based) where the resource is within working hours. */
  openSlots: z.array(z.number().int().nonnegative()),
});
...
/** Clinic-local open interval as `HH:mm` (seconds optional). */
...
export const suggestAppointmentsBodySchema = z
  .object({
    patientId: z.uuid(),
    date: clinicDateSchema,
    ...
    /** Clinic-local window the patient can attend; entire chain must fit inside. */
    patientWindow: availabilityWindowSchema.optional(),
  })
  .superRefine((body, ctx) => {
    if (body.patientWindow === undefined) return;
    const start = body.patientWindow.startsAt.slice(0, 5);
    const end = body.patientWindow.endsAt.slice(0, 5);
    if (start >= end) {
      ctx.addIssue({
        code: 'custom',
        message: 'Patient availability must end after it starts.',
        path: ['patientWindow'],
      });
    }
  });
...
export const suggestAppointmentsResponseSchema = z.object({
  ...
  /** Populated when `candidates` is empty — other days that can fit the chain. */
  alternateDates: z
    .array(
      z.object({
        date: clinicDateSchema,
...
export const currentUserSchema = z.object({
  ...
  role: userRoleSchema,
  /** Set for CLINICIAN logins linked to a doctor resource. */
  resourceId: z.uuid().nullable(),
});

export const setResourceDayAvailabilityBodySchema = z.object({
  date: clinicDateSchema,
  /** One or more open intervals for this calendar date (e.g. 08:00–08:45, 09:00–09:15). */
  windows: z.array(availabilityWindowSchema).min(1).max(8),
});

export const getResourceAvailabilityResponseSchema = z.object({
  resourceId: z.uuid(),
  date: clinicDateSchema,
  /** True when windows were saved for this date; false when showing a weekday template. */
  savedForDate: z.boolean(),
  windows: z.array(availabilityWindowSchema),
});

export type SetResourceDayAvailabilityBody = z.infer<
  typeof setResourceDayAvailabilityBodySchema
>;

export type GetResourceAvailabilityResponse = z.infer<
  typeof getResourceAvailabilityResponseSchema
>;
 ...
 * refresh token is never in a body ΓÇö it travels as an httpOnly cookie, so
